package org.lushplugins.regrowthsmp.module.abilities.ability;

public class AbilityTypes {
    public static final String BETTER_POTIONS = "better_potions";
    public static final String BOAT_BOOST = "boat_boost";
    public static final String CONTACT_INVULNERABILITY = "contact_invulnerability";
    public static final String FRIENDLY_MOBS = "friendly_mobs";
    public static final String GROWTH_DANCE = "growth_dance";
    public static final String ORE_CRUNCHER = "ore_cruncher";
    public static final String STEP = "step";
    public static final String TREE_CHOMPER = "tree_chomper";
}
